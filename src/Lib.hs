...
-- day 2 part 1
data Move = Up | Down | Forward

eval :: [(Move, Int)] -> (Int, Int)
eval = foldl evalOne (0,0)
    where
        evalOne (d, f) (Up, x) = (d-x, f)
        evalOne (d, f) (Down, x) = (d+x, f)
        evalOne (d, f) (Forward, x) = (d, f+x)

parseLine :: String -> (Move, Int)
parseLine line =
    case words line of
...
main_3 =
    ...
    return . (\(x,y)->x*y) . eval . map parseLine . lines >>=
    print


-- day 2 part 3

evalAim :: [(Move, Int)] -> Int
